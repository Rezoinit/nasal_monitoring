...
# Enable live detection — downsample to 8 Hz so detector sees peak-to-peak values
...
def on_reading(r):
    ...
    t1 = f"{thresh['mic1_threshold']:.0f}" if thresh else "?"
    t2 = f"{thresh['mic2_threshold']:.0f}" if thresh else "?"
    print(
        f"seq={r.seq:5d}  "
        f"mic1={r.mic1:4d}(thr:{t1})  "
        f"mic2={r.mic2:4d}(thr:{t2})  "
